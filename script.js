...
async function loadSupabaseEvents() {
  ...
  try {
    ...
    eventGrid.innerHTML = prepareEvents(events).map(renderEventCard).join('');
...
function prepareEvents(events) {
  const normalizedEvents = events.map((event) => ({
    ...event,
    title: event.title?.trim().toUpperCase() === 'VLADOVOROT' ? 'VLADOVOROT' : event.title,
  }));

  const eventOrder = {
    '#ДунІЛДО': 1,
    VLADOVOROT: 2,
    'VLADOVOROT CLASSIC': 3,
    DunchLand: 4,
    "Двоє, не рахуючи dunchoff'a": 5,
  };

  normalizedEvents.sort((firstEvent, secondEvent) =>
    (eventOrder[firstEvent.title] ?? 0) - (eventOrder[secondEvent.title] ?? 0),
  );
